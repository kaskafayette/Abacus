...
13. **Payee Metadata maintenance page improvements.** Several upgrades to the Payee Metadata tab (`ui/maintenance.py` `_payee_metadata`):
    - **Search box at the top** to filter the grid by category, subcategory, or a particular payee/vendor, so you can jump to what you want instead of scrolling the whole table.
    - **Total-dollars column per payee** — sum of transaction amounts for each normalized payee — so a review can focus on the payees that actually move the needle and skip the trivial ones. (Join `payee_metadata.normalized_name` to `transactions.payee`, sum `amount`.)
    - **Conditional subcategory dropdown** — the Subcategory column is currently free text / a flat list; make it cascade from the chosen Category like the Categorize tab and the Split editor do (reuse the `subcats_map` + `JsCode` cellEditor pattern).
    - **User-friendly column headings** — the grid currently shows the raw DB column names (`normalized_name`, `category_override`, `subcategory_override`, `tax_flags_override`, etc.); relabel them to readable titles (Payee, Category, Subcategory, Tax Flags, ...).
14. **Better handling of check rows in the normalization flow.** Three related fixes, all in the check-payee path:

    **(a) Add a check-specific extractor** (parallel to `_extract_zelle_payee` and `_extract_online_payment_payee` in `processing/normalize.py`). Chase sometimes embeds the real payee in a check row's description — e.g. `CHECK # 1040    STATE FARM RO 08 PYMT`. Today the generic substring matcher still finds `STATE FARM` if there's already a `State Farm` normalization rule, so the row auto-matches. But if there's no rule (a check to a new payee), the row falls into unmatched and `auto_suggest_payee` returns a garbled suggestion like `Check # 1040 State Farm Ro 08 Pymt` because it doesn't understand the `CHECK # ####` prefix. The new extractor should strip the `CHECK # ####\s+` prefix (and any trailing generic suffix like ` PYMT`) before matching and suggestion, so:
      - If the stripped remainder matches an existing rule (`STATE FARM`), the row auto-matches without user intervention.
      - If not, the `suggested_name` becomes the clean stripped remainder (`State Farm Ro 08 Pymt`), which the user can edit down to `State Farm` in one small step instead of hand-cleaning the whole check prefix.

    **(b) Don't create dead normalization rules for checks.** Today, when the user assigns a payee to a check row on the Payee Normalization tab and clicks Commit All, the code writes a `payee_normalization` rule with pattern = the cleaned description (e.g. `CHECK 1002` → John Doe). Because every check's description is unique (the check number is embedded), that rule can never match another transaction — it's dead the moment it's written. Over years, the rules table accumulates hundreds of these one-off entries that clutter the Payee Normalization maintenance grid. Fix: when the source row has a non-null `check_number`, **skip the `insert_payee_normalization` call** in `ui/normalize.py` `_payee_normalization`'s Commit All branch and just call `queries.update_transaction` with the payee.
